...
function getAk(){
  ...
  if(!akId || !akSecret){
    ...
    if(!fs.existsSync(credentialsFile)){
      console.error('\x1b[31m','can not find access_key_id and access_key_secret, check if it includes in process.env or ~/fcli/config.yaml','\x1b[m');
      process.exit(1);
    }
...
function getOssOptions (){
  ...
  if(!options.config){
    ...
    if(!fs.existsSync(defaultConfigPath)){
      console.error('\x1b[31m','can not find config file oss-publish.json','\x1b[m');
      process.exit(1);
    }else {
