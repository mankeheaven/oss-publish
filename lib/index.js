...
import Pool from "./utils/task-pool.js";
 ...
 * options.poolLimit, optionial default 50
 * options.poolTimeout, optionial default 10000 10秒
 **/
...
async function publish(store, options) {
	...
	console.info("开始上传...");
	const pool = new Pool({
		limit: options?.poolLimit || 50,
		timeout: options?.poolTimeout || 10000,
	});
	const taskPut = pool.promise.wrap(put);
	for (; i < length; i++) {
		let m = files2publish[i].name;
		taskPut(m, files2publish[i].filePath)
			.then((result) => {
				if (result === true) {
					successCount++;
				} else {
